...
self.addEventListener("message", async (e) => {
  ...
  if (type === "load") {
    ...
    if (!transcriber || currentModel !== requestedModel) {
      ...
      try {
        ...
        // Fix: send explicit error so UI can show user-facing message instead of infinite "initializing"
        postMessage({ type: "error", message: `Model load failed: ${err instanceof Error ? err.message : String(err)}` });
        transcriber = null;
        currentModel = null;
      }
    } else {
  ...
  if (type === "transcribe") {
    ...
    try {
      ...
      // 2. Simplified pipeline call as per Transformers.js docs with strict language to avoid 10x slowdown from auto-detect
